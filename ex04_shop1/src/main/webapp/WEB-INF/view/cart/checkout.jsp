...
<table>
	...
	<c:forEach items="${sessionScope.CART.itemSetMap}" var="entry">
		<tr><td>${entry.value.item.name}</td><td>${entry.value.item.price}</td>
				<td>${entry.value.quantity}</td>
				<td>${entry.value.item.price * entry.value.quantity}</td></tr>
	</c:forEach>
